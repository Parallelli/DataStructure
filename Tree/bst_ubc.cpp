...
public:
    ...
    bool find_value(string value) const;
private:
    string data;
    TreeNode* left;
    TreeNode* right;
...
bool BinarySearchTree::find(string data) const
{
    if (root == NULL) return 0;
    else if (root->find_value(data)) return 1;
    else return 0;
}
...
bool TreeNode::find_value(string value) const
{
    if (value < data)
    {
        if (left == NULL) return false;
        else return left->find_value(value);
    }
    else if (data < value)
    {
        if (right == NULL) return false;
        else return right->find_value(value);
    }
    else
        return true;
}
